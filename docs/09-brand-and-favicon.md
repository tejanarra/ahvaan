...
## Part A2 — The second rename: Gatherie → **Ahvaan**
...
| Avahan | briefly chosen, then corrected by the host to the "Ahvaan" spelling (below) — same word, different transliteration |

### The name: **Ahvaan** — chosen by the host 2026-07-30
- **Meaning**: a transliteration of the Sanskrit/Hindi word आह्वान (āhvāna) —
  ...
  mark change.
- **Known associations, disclosed rather than hidden**: "Ahvaan" is also used
  by a small India-based tech-services company (Ahvaan Solutions Pvt Ltd /
  ahvaan.io) and a couple of small handicraft/fashion shops — none in the
  ...
  marketing spend.
- **Tagline**: "Ahvaan — RSVP made easy" — used in `<title>`/meta description
...
- **Usage rules**: "Ahvaan" capitalized in prose; all-lowercase "ahvaan" in
  the wordmark lockup itself (same convention Gatherie used) — corrected
  2026-08-02 after two false starts (first all-caps "AHVAAN", then
  title-case) — see `components/brand.tsx`.
...
Ahvaan" footer line, `package.json` name. **Not renamed**: git branch, folder
...
text and name-dependent copy moved from "gatherie" to "ahvaan".
...
5. The wordmark lockup component (`components/brand.tsx`): mark + "ahvaan"
   in the display serif (Fraunces, WONK 1, −0.02em tracking), mark sized to
   the x-height ×1.6, 8px gap. Used in Studio header, auth pages, marketing
   header/footer, email header.
...
project memory ("Ahvaan, formerly Gatherie, formerly Invitely" appears only
here). Doc 04's
